...
import {CHAT_STORAGE_KEY,LEGACY_CHAT_STORAGE_KEY,CHAT_GREETING,createChatState,activeChat,ensureProductChat,appendChatMessage,sortedChats,latestChat,savedChats,ensureStrategyChats,greetChat,chatSuggestions} from './chat-model.mjs?v=120';
import {attachColumnResizer,chatColumnWidth,chatColumnLimits} from './column-resizer.mjs?v=120';
import {createVoiceSession} from './audio-session.mjs?v=131';
...
const voice=createVoiceSession({host:window,client,getPreferences:()=>preferences,onChange:({mode,phase,message})=>{
  ...
  byId('chat-mic').setAttribute('aria-pressed',String(mode==='mic'));byId('chat-mic').setAttribute('aria-label',mode==='mic'?'Stop and send recording':'Record a voice message');
  syncLauncher();
},onLevel:level=>launcher.style.setProperty('--voice-level',level.toFixed(3)),onTranscript:(text,options)=>sendTurn(text,{spoken:true,signal:options.signal})});
...
async function sendTurn(text,{spoken=false,signal,retry=false}={}){
 if(activeRequest)return '';if(!spoken)voice.stop();const target=room(),id=target.id;
...
function refreshComposer(){const busy=activeRequest?.id===state.active;submit.disabled=!busy&&!input.value.trim();submit.setAttribute('aria-label',busy?'Stop response':'Send message');submit.dataset.busy=String(Boolean(busy));submit.querySelector('path').setAttribute('d',busy?'M6 6h12v12H6z':'M12 20V4m-7 7 7-7 7 7');input.style.height='44px';input.style.height=Math.min(84,Math.max(44,input.scrollHeight))+'px';}
...
function syncLauncher(){
  ...
  const activity=mode==='off'?'':mode==='mic'?(phase==='listening'?'recording continues':phase==='ready'?'recording ready to send':'voice message '+phase):mode==='live'?'live voice '+phase:'reading aloud';
...
function syncMenu(){if(!menu.hidden){if(!pageMode)closeChat({restoreFocus:false});}syncLauncher();}
...
document.addEventListener('dolce:ppc-controls-change',event=>{if(event.detail.open){if(!pageMode)closeChat({restoreFocus:false});}requestAnimationFrame(updateViewport);});
...
function syncWorkspace(){
  ...
  if(entering)selectRoom(voice.getState().mode!=='off'||activeRequest?state.active:latestChat(state).id);else setView();byId('chat-column-resizer').hidden=false;syncMenu();refreshListWidth();updateViewport();
}
...
form.addEventListener('submit',event=>{event.preventDefault();if(!pageMode||view!=='messages')return;if(activeRequest){cancelRequest();return;}sendTurn(input.value);});
