...
export function createVoiceSession({host=globalThis,getPreferences=()=>({}),client,onChange=()=>{},onLevel=()=>{},onTranscript=async()=>''}={}){
 ...
 async function process(blob,id){
  ...
  try{
   ...
   changed('thinking','Responding');const reply=await onTranscript(text,{signal:controller.signal});if(!alive(id))return;
   if(reply&&getPreferences().readReplies!==false)await speak(reply,id);if(!alive(id))return;
 ...
 function startRecorder(id){
  ...
  try{
   ...
   current.onstop=()=>{if(!alive(id)||recorder!==current)return;const blob=new host.Blob(chunks,{type:current.mimeType||type||'audio/webm'});recorder=null;onLevel(0);
    ...
    else{for(const track of stream?.getTracks()||[])track.stop();stream=null;source?.disconnect();source=null;analyser=null;if(sendOnStop)process(blob,id);else{readyBlob=blob;changed('ready','Recording ready — press Stop to send');}}
   };
   current.start(150);changed('listening',mode==='mic'?'Recording — press Stop to send':'Listening');
   {
   ...
   }if(mode==='mic')later(()=>{if(alive(id)&&recorder===current&&current.state==='recording')current.stop();},60000);
  }catch(error){fail(error,id);}
 }
 ...
 function toggleMic(){
  if(mode==='mic'){
   ...
   if(recorder?.state==='recording'){sendOnStop=true;clearTimers();changed('finishing','Sending recording');recorder.stop();}else stop();
  }else start('mic');
 }
 ...
 return{stop,toggleMic,toggleLive,preview,getState:()=>({mode,phase})};
}
